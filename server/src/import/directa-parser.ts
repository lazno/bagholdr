...
export type DirectaTransactionType =
	| 'Buy'
	| 'Sell'
	| 'Commissions'
	| 'Wire transfer payment'
	| 'Cap.gain tax'
	| 'Portfolio stamp duty*'
	| 'Etf withholding tax'
	| 'Bond accrd int wd'
	| 'Bond accrd int wd tax'
	| 'Bonds coupon pmt'
	| 'Bonds coupon tax'
	| 'Bond accrd int pmt'
	| 'Bond accrd int pmt tax'
	| 'Disagio debt w/t'
	| 'Disagio credit w/t';
 ...
 */
function convertItalianDate(dateStr: string): string | null {
 ...
 */
function extractAccountName(headerLine: string): string | null {
 ...
 */
function isImportableTransaction(type: string): type is 'Buy' | 'Sell' | 'Commissions' {
 ...
 */
function mapTransactionType(type: 'Buy' | 'Sell' | 'Commissions'): 'Buy' | 'Sell' | 'Commission' {
	if (type === 'Commissions') return 'Commission';
	return type;
}
 ...
 */
export function parseDirectaCSV(content: string): DirectaParseResult {
