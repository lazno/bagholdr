...
import { holdings, assets, sleeves, sleeveAssets, portfolios, priceCache, portfolioRules, globalCash, orders, dailyPrices, fxCache } from '../../db/schema';
import { calculateBand, evaluateStatus } from '../../utils/bands';
/**
 * Recursively calculate total value for a sleeve including all descendants
 */
function calculateSleeveTotal(sleeveId, childrenMap, directValueMap) {
...
export const valuationRouter = router({
    ...
    getPortfolioValuation: publicProcedure
        ...
        const lastSyncAt = cachedPrices.length > 0
        ...
        const nativeCostByIsin = new Map();
        const ordersByIsin = new Map();
        ...
        for (const [isin, isinOrders] of ordersByIsin) {
            ...
            const sortedOrders = [...isinOrders].sort((a, b) => a.orderDate.getTime() - b.orderDate.getTime());
            let totalQty = 0;
        ...
        const assetValuationsPartial = allHoldings.map((h) => {
            ...
            const valueEur = priceEur !== null ? priceEur * h.holding.quantity : h.holding.totalCostEur;
            ...
            const avgPriceNative_fromEurViaImpliedFx = impliedHistoricalFxRate && impliedHistoricalFxRate !== 0
                ? avgPriceEur / impliedHistoricalFxRate
                : 0;
            // Approach 3: From EUR, converted via current FX rate
            const avgPriceNative_fromEurViaCurrentFx = currentFxRate && currentFxRate !== 0 ? avgPriceEur / currentFxRate : null;
            const calculations = {
        ...
        const assetValuations = assetValuationsPartial.map((a) => ({
            ...a,
        ...
        const sleeveAllocations = portfolioSleeves.map((sleeve) => {
            ...
            // Dual percentages
            const actualPercentInvested = investedValueEur > 0 ? (sleeveTotal / investedValueEur) * 100 : 0;
        ...
        const missingSymbolAssets = allHoldings
        ...
        const now = Date.now();
        const stalePriceAssets = allHoldings
            .map((h) => {
            const lookupKey = h.asset.yahooSymbol ?? h.asset.ticker;
    ...
    getChartData: publicProcedure
        ...
        const firstOrderDate = filteredOrders.reduce((earliest, order) => (order.orderDate < earliest ? order.orderDate : earliest), filteredOrders[0].orderDate);
        ...
        const pricesResult = yahooSymbols.size > 0
            ? await ctx.db
                .select()
                .from(dailyPrices)
                .where(and(inArray(dailyPrices.ticker, Array.from(yahooSymbols)), gte(dailyPrices.date, startDateStr), lte(dailyPrices.date, endDateStr)))
            : [];
        ...
        const priceByTickerDate = new Map();
        ...
        const sortedOrders = [...filteredOrders].sort((a, b) => a.orderDate.getTime() - b.orderDate.getTime());
        ...
        function getFxRateToEur(currency) {
            if (currency === 'EUR')
                return 1;
        ...
        function getPriceForDate(isin, date, sortedDatesForTicker) {
        ...
        const sortedDatesByTicker = new Map();
        ...
        const cachedPrices = await ctx.db.select().from(priceCache);
        const currentPriceMap = new Map(); // yahooSymbol -> priceEur
        // Also derive FX rates from priceCache for consistency with displayed values
        const derivedFxRateMap = new Map(); // yahooSymbol -> EUR/native rate
    ...
    getHistoricalReturns: publicProcedure
        ...
        const getComparisonDate = (period, firstOrderDateStr) => {
        ...
        const periods = ['today', '1w', '1m', '6m', 'ytd', '1y', 'all'];
        // Get all non-archived assets with their Yahoo symbols
        const allAssets = await ctx.db.select().from(assets).where(eq(assets.archived, false));
        const assetMap = new Map(allAssets.map((a) => [a.isin, a]));
        const nonArchivedIsins = new Set(allAssets.map((a) => a.isin));
        ...
        const allDatesNeeded = new Set([todayStr]);
        ...
        const pricesResult = yahooSymbols.size > 0
            ? await ctx.db
                .select()
                .from(dailyPrices)
                .where(and(inArray(dailyPrices.ticker, Array.from(yahooSymbols)), gte(dailyPrices.date, lookbackDateStr), lte(dailyPrices.date, todayStr)))
            : [];
        ...
        // Helper: get price for an asset on a date (or nearest prior date)
        function getPriceForDate(isin, targetDate) {
            const asset = assetMap.get(isin);
            if (!asset?.yahooSymbol)
                return null;
            const tickerPrices = priceByTickerDate.get(asset.yahooSymbol);
        ...
        function calculatePortfolioValue(date, usePriceCache) {
            const positions = getPositionsForDate(date);
            let totalValue = 0;
            for (const [isin, position] of positions) {
                if (position.quantity <= 0)
        ...
        const totalCostBasis = Array.from(currentPositions.values()).reduce((sum, pos) => sum + pos.costBasisEur, 0);
        ...
        const holdingIsins = Array.from(currentHoldings.keys()).filter((isin) => (currentHoldings.get(isin)?.quantity ?? 0) > 0);
        // Helper: find first order date for an asset
        function getAssetFirstOrderDate(isin) {
        ...
        for (const period of periods) {
            ...
            if (firstOrderDate && new Date(comparisonDate) >= firstOrderDate && startValue > 0) {
                ...
                const { annualizedReturn, compoundedReturn, netCashFlow, cashFlowCount, periodYears } = calculateMWR(comparisonDate, todayStr, startValue, currentValue, cashFlows);
                ...
                for (const isin of holdingIsins) {
                    ...
                    let compoundedReturnAsset = null;
                    let annualizedReturnAsset = null;
                    let periodYearsAsset = null;
                    let absoluteReturnAsset = null;
 ...
 */
function calculateTWR(startDate, endDate, cashFlows, getPortfolioValueAtDate) {
    ...
    for (const flowDate of uniqueDates) {
        ...
        const valueBeforeFlow = dayBefore >= subPeriodStart
 ...
 */
function calculateMWR(startDate, endDate, startValue, endValue, cashFlows) {
