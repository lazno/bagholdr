...
import { syncHistoricalData, getHistoricalPrices, getDividendEvents, getTickersNeedingSync, needsHistoricalSync, syncIntradayData, getIntradayPrices, needsIntradaySync, getTickersNeedingIntradaySync, ensureTickerMetadata } from '../../oracle/historical';
...
export const oracleRouter = router({
    ...
    resolveYahooSymbols: publicProcedure
        ...
        try {
            ...
            const now = new Date();
            const insertedSymbols = await Promise.all(symbols.map(async (s) => {
                const id = randomUUID();
                await ctx.db.insert(yahooSymbols).values({
                    id,
    ...
    getHistoricalPrices: publicProcedure
        ...
        const candles = await getHistoricalPrices(ctx.db, asset.yahooSymbol, input.startDate, input.endDate);
        return {
            isin: input.isin,
    ...
    getDividends: publicProcedure
        ...
        const dividends = await getDividendEvents(ctx.db, asset.yahooSymbol, input.startDate);
        return {
            isin: input.isin,
            ticker: asset.yahooSymbol,
            dividends
    ...
    getTickersNeedingHistoricalSync: publicProcedure.query(async ({ ctx }) => {
        ...
        const tickerToIsin = new Map(holdingsWithAssets
