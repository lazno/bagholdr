 ...
 */
async function validateSiblingBudgets(db, portfolioId, parentSleeveId, excludeSleeveId) {
    const query = db
        .select()
        .from(sleeves)
        .where(parentSleeveId === null
    ...
    const total = filtered.reduce((sum, s) => sum + s.budgetPercent, 0);
    return {
...
export const sleevesRouter = router({
    ...
    create: publicProcedure
        ...
        const siblings = await ctx.db
            .select()
            .from(sleeves)
            .where(input.parentSleeveId === null
    ...
    delete: publicProcedure
        ...
        const sleevesToDelete = [];
        const collectDescendants = async (sleeveId) => {
