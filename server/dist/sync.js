...
export async function runPriceSyncJob() {
    ...
    try {
        ...
        for (let i = 0; i < heldAssets.length; i++) {
            ...
            let historicalStatus = needsHistorical ? 'done' : 'skipped';
            if (needsHistorical) {
                serverEvents.emitSyncItemUpdate({
                    job: 'price',
                    ticker,
            ...
            let intradayStatus = needsIntraday ? 'done' : 'skipped';
            if (needsIntraday) {
                serverEvents.emitSyncItemUpdate({
                    job: 'price',
                    ticker,
 ...
 */
export function triggerPriceSync() {
