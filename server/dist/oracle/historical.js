 ...
 */
export async function syncHistoricalData(db, ticker) {
    const now = new Date();
    try {
 ...
 */
export async function needsHistoricalSync(db, ticker) {
    const [meta] = await db
        .select()
        .from(tickerMetadata)
 ...
 */
export async function getTickersNeedingSync(db, tickers) {
    const today = new Date().toISOString().split('T')[0];
    const needsSync = [];
 ...
 */
export async function ensureTickerMetadata(db, tickers) {
    let created = 0;
    for (const ticker of tickers) {
 ...
 */
export async function getHistoricalPrices(db, ticker, startDate, endDate) {
    ...
    const results = await db
        ...
        .from(dailyPrices)
        .where(and(eq(dailyPrices.ticker, ticker), gte(dailyPrices.date, start), lte(dailyPrices.date, end)))
 ...
 */
export async function getDividendEvents(db, ticker, startDate) {
    let query = db
        .select({
    ...
    if (startDate) {
        query = db
            ...
            .from(dividendEvents)
            .where(and(eq(dividendEvents.ticker, ticker), gte(dividendEvents.exDate, startDate)));
    }
 ...
 */
export async function getTickerMetadata(db, ticker) {
    const [meta] = await db
        .select()
        .from(tickerMetadata)
        .where(eq(tickerMetadata.ticker, ticker))
 ...
 * @returns Sync statistics
 */
export async function syncIntradayData(db, ticker) {
    const now = new Date();
    try {
        ...
        const purgeResult = await db
            ...
            .where(and(eq(intradayPrices.ticker, ticker), lt(intradayPrices.timestamp, fiveDaysAgo)));
 ...
 */
export async function getIntradayPrices(db, ticker) {
    const results = await db
        .select({
 ...
 */
export async function needsIntradaySync(db, ticker) {
    const [meta] = await db
        .select()
        .from(tickerMetadata)
        .where(eq(tickerMetadata.ticker, ticker))
 ...
 */
export async function getTickersNeedingIntradaySync(db, tickers) {
    const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
    const needsSync = [];
    for (const ticker of tickers) {
        const [meta] = await db
