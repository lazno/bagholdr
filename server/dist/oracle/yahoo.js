 ...
 */
export async function fetchAllSymbolsFromIsin(isin) {
    ...
    if (!response.ok) {
        throw new YahooFinanceError(`HTTP error fetching symbol: ${response.status}`, 'HTTP_ERROR');
    }
 ...
 */
export async function fetchSymbolFromIsin(isin) {
...
export async function fetchPriceData(symbol) {
    ...
    if (!response.ok) {
        throw new YahooFinanceError(`HTTP error fetching price: ${response.status}`, 'HTTP_ERROR');
    }
    const data = await response.json();
 ...
 */
export async function fetchHistoricalData(symbol, range = '10y') {
    ...
    if (!response.ok) {
        throw new YahooFinanceError(`HTTP error fetching historical data: ${response.status}`, 'HTTP_ERROR');
    }
    const data = await response.json();
    ...
    if (actualGranularity && actualGranularity !== '1d') {
        console.warn(`[Yahoo] ${symbol}: Requested daily data but got ${actualGranularity} granularity. ` +
 ...
 */
export async function fetchIntradayData(symbol, range = '5d') {
    ...
    if (!response.ok) {
        throw new YahooFinanceError(`HTTP error fetching intraday data: ${response.status}`, 'HTTP_ERROR');
    }
    const data = await response.json();
    if (!data.chart?.result?.[0]) {
 ...
 */
export function adjustConversionRate(currency, rate) {
 ...
 */
export async function fetchPriceInEur(isin, knownTicker) {
