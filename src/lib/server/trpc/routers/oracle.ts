...
import { assets, holdings, priceCache, fxCache, yahooSymbols, dailyPrices, intradayPrices, dividendEvents, tickerMetadata } from '$lib/server/db/schema';
import { getPrice, getFxRate, clearExpiredCache, clearAllCache } from '$lib/server/oracle/cache';
import { fetchAllSymbolsFromIsin, YahooFinanceError } from '$lib/server/oracle/yahoo';
import {
	syncHistoricalData,
	getHistoricalPrices,
	getDividendEvents,
...
} from '$lib/server/oracle/historical';
...
export const oracleRouter = router({
	 ...
	 * 
	 * @param maxAgeMs - Maximum age in milliseconds before a price is considered stale.
	 *                   If not provided, defaults to PRICE_CACHE_TTL_MS (6 hours).
	 *                   Pass the auto-refresh interval to get assets that need refresh.
	 */
